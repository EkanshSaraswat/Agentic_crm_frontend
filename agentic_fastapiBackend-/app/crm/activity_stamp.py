...
async def stamp_last_activity(company_id: Optional[str] = None, contact_id: Optional[str] = None, deal_id: Optional[str] = None):
    await activity_stamp.stamp_last_activity(company_id, contact_id, deal_id)

async def recompute_after_delete(target_type: str, ids: list):
    await activity_stamp.recompute_after_delete(target_type, ids)

async def stamp_stage_change(deal_id: str, new_stage: str):
    pass
